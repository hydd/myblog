...
Mac上安装可以选择图形化方式和终端安装，此处使用终端安装。通过终端安装需要mac已经安装包管理工具[homebrew](https://www.jianshu.com/p/de6f1d2d37bf)，若不确定是否安装的话可以通过在终端执行命令`brew -v `来查看。若结果如下所示，即homebrew已经安装。
